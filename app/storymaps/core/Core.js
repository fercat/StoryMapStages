define(["esri/map",
		...
		Extent)
	{
		...
		function init(mainView)
		{
			...
			if ( ! Helper.browserSupportHistory() && urlParams.ieredirected == 'undefined' ) {
			...
			esriConfig.defaults.io.timeout = APPCFG.TIMEOUT_VIEWER_REQUEST;
		...
		function initStep3()
		{
			...
			else if ( isProd() && ! Helper.getPortalUser() )
				redirectToSignIn();
			...
			app.portal.on("load", function(){
				app.portal.signIn().then(
					function() {
						...
						if (! app.data.checkUserItemPrivileges()) {
		...
		function webMapInitCallback(response)
		{
			...
			app.header.init((APPCFG.EMBED || urlParams.embed || urlParams.embed === ''),
				title,
				subtitle,
				appColors[0],
				logoURL,
